...
data_file_segmented = os.path.join(processed_data_dir, "segmented_data.h5")
...
if os.path.exists(data_file_csv):
    ...
    aggregate_data.drop(columns=columns_to_drop, inplace=True, errors="ignore")
...
if os.path.exists(data_file_segmented):
    ...
    with h5py.File(data_file_segmented, "w") as f:
        f.create_dataset("X", data=input_segmented_data, compression="gzip")
        f.create_dataset("y", data=output_segmented_data, compression="gzip")
